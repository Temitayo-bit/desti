...
export default function BookingsPage() {
  ...
  async function handleCompleteBooking(bookingId: string) {
    ...
    try {
      ...
      if (res.ok) {
        setActionNotice({ type: "success", text: "Trip marked as complete! You can now rate your experience." });
        setShowRatingForm(true);
      } else {
        const data = await res.json().catch(() => null);
        setActionNotice({ type: "error", text: data?.message ?? "Failed to complete trip." });
      }
    } catch {
  ...
  async function handleSubmitRating(bookingId: string) {
    ...
    try {
      ...
      if (res.ok) {
        setActionNotice({ type: "success", text: "Rating submitted! Thank you for your feedback." });
        setShowRatingForm(false);
        setSelectedTrip(null);
      } else {
        const data = await res.json().catch(() => null);
        setActionNotice({ type: "error", text: data?.message ?? "Failed to submit rating." });
      }
    } catch {
      setActionNotice({ type: "error", text: "Network error. Please try again." });
    } finally {
  ...
  return (
    <ProtectedShell activeNav="bookings">
          ...
          >
            <div className="p-6 md:p-8 flex-1">
              ...
              <div className="mt-8 pt-6 border-t border-zinc-100 space-y-4">
                {/* Rating form */}
                {showRatingForm && selectedTrip && (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 space-y-4">
                    <h4 className="font-bold text-emerald-900 text-lg">Rate Your Trip</h4>
                    <p className="text-sm text-emerald-700">How was your experience with the driver?</p>
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          ...
                          className={`text-3xl transition-transform hover:scale-110 ${star <= ratingScore ? "text-amber-400" : "text-zinc-300"}`}
                        >
                          ★
                        </button>
                      ))}
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-white border border-emerald-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl p-3 text-zinc-900 outline-none text-sm"
                    />
                    <div className="flex gap-3">
                      <button
                        onClick={() => void handleSubmitRating(selectedTrip.id)}
                        disabled={ratingSubmitting}
                        className="px-6 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-medium rounded-xl transition-colors disabled:opacity-60"
                      >
                ...
                <div className="flex gap-3 justify-end items-center">
                  ...
                  {viewerIsDriverForSelectedTrip && selectedTrip?.status === "CONFIRMED" && !showRatingForm && (
                    <button
                      onClick={() => void handleCompleteBooking(selectedTrip.id)}
                      disabled={completing}
                      className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors disabled:opacity-60"
                    >
                      {completing ? "Completing..." : "Mark Complete"}
                    </button>
                  )}
                  {/* Rider: Rate Trip */}
                  {!viewerIsDriverForSelectedTrip && selectedTrip && !showRatingForm && (
                    <button
                      onClick={() => setShowRatingForm(true)}
                      className="px-5 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 font-medium rounded-xl transition-colors border border-amber-200"
                    >
                      Rate Trip
                    </button>
                  )}
