...
import { Calendar, Car, Map, MapPin, Search, User } from "lucide-react";
...
// --- Sidebar / API filters (MVP2 booleans: only send when set; strict on server) ---
interface SidebarApiFilters {
...
const EMPTY_SIDEBAR_API: SidebarApiFilters = {
  musicPreference: "",
  hasAc: "",
  hasTrunkSpace: "",
  vehicleType: "",
  ...
  if (currentView === "my") {
    return (
      <ProtectedShell
        ...
        topNavActive="browse"
      >
        <div className="rounded-3xl bg-[#006837] px-6 py-8 text-white shadow-sm md:px-8 md:py-10">
          <h1 className="text-2xl font-bold tracking-tight md:text-3xl">
            Your posted rides
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-white/90">
            Manage trips you are offering. Switch to Browse Rides to find seats on
            other drivers&apos; trips.
          </p>
        </div>
        <RidesViewToggle activeView="my" />
        <div className="mt-2 grid min-w-0 gap-6 lg:grid-cols-12">
          <aside className="min-w-0 space-y-4 lg:col-span-4 xl:col-span-3">
            <div className="rounded-2xl border border-[#006837]/20 bg-[#C4E5D4]/50 p-4 text-sm text-[#0d3d2e]">
              ...
              <p className="mt-2 leading-relaxed">
                Rides with &apos;Stetson Verified&apos; badges are driven by
                students with cleared campus background checks.
              </p>
            </div>
          </aside>
          <div className="min-w-0 lg:col-span-8 xl:col-span-9">
            <MyRidesView embedded />
          </div>
        </div>
        <BrowsePageFooter />
      </ProtectedShell>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {(
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setTimeWindow((t) => (t === tw.id ? null : tw.id))
                    }
