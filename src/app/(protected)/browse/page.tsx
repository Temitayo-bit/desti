...
function buildApiQueryString(filters: AdvancedFilters): string {
  ...
  if (filters.musicPreference) params.set("musicPreference", filters.musicPreference);
  if (filters.hasAc) params.set("hasAc", filters.hasAc);
  if (filters.hasTrunkSpace) params.set("hasTrunkSpace", filters.hasTrunkSpace);
...
export default function BrowseRidesPage() {
  ...
  const [successState, setSuccessState] = useState<"booking" | "edit" | "offer" | "stopRequest" | null>(null);
  ...
  // Stop request state
  const [showStopRequestForm, setShowStopRequestForm] = useState(false);
  const [stopRequestSubmitting, setStopRequestSubmitting] = useState(false);
  const [stopRequestError, setStopRequestError] = useState<string | null>(null);
  const [stopPickupText, setStopPickupText] = useState("");
  const [stopDropoffText, setStopDropoffText] = useState("");
  const [stopRiderNote, setStopRiderNote] = useState("");
    ...
    async function fetchRides() {
      try {
        ...
        if (!res.ok) {
          ...
          console.error(`Rides API returned ${res.status}:`, errBody);
  ...
  const closeRideModal = () => {
    ...
    setOfferPriceDollars("");
    setOfferError(null);
    setShowStopRequestForm(false);
    setStopPickupText("");
    setStopDropoffText("");
    setStopRiderNote("");
    setStopRequestError(null);
  ...
  const handleSendRideOffer = async (rideId: string) => {
    if (offerSubmitting) return;
    setOfferError(null);

    const dollars = Number(offerPriceDollars);
    if (!Number.isFinite(dollars) || dollars <= 0) {
      setOfferError("Please enter a valid price.");
      return;
    }
    const offeredPriceCents = Math.round(dollars * 100);
    ...
    try {
      ...
      const res = await fetch(`/api/rides/${rideId}/offers`, {
        ...
        body: JSON.stringify({ offeredPriceCents }),
      });

      if (res.ok) {
        setSuccessState("offer");
        setTimeout(() => closeRideModal(), 2500);
      } else {
  ...
  const handleSendStopRequest = async (rideId: string) => {
    if (stopRequestSubmitting) return;
    setStopRequestError(null);

    if (!stopPickupText.trim() || !stopDropoffText.trim()) {
      setStopRequestError("Pickup and dropoff locations are required.");
      return;
    }

    try {
      setStopRequestSubmitting(true);
      const res = await fetch(`/api/rides/${rideId}/stop-requests`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Idempotency-Key": crypto.randomUUID(),
        ...
        body: JSON.stringify({
          requestedPickupText: stopPickupText.trim(),
          requestedPickupLatitude: 29.0349,
          requestedPickupLongitude: -81.3026,
          requestedDropoffText: stopDropoffText.trim(),
          requestedDropoffLatitude: 29.0349,
          requestedDropoffLongitude: -81.3026,
          riderNote: stopRiderNote.trim() || null,
        }),
      });

      if (res.ok) {
        setSuccessState("stopRequest");
        setTimeout(() => closeRideModal(), 2500);
      } else {
        const data = await res.json().catch(() => null);
        setStopRequestError(data?.message ?? "Failed to send stop request.");
      }
    } catch {
      setStopRequestError("Network error. Please try again.");
    } finally {
      setStopRequestSubmitting(false);
    }
  };
              ...
              <div className="flex flex-col gap-4">
                {filteredRides.map((ride) => (
                  ...
                  >
                    ...
                    {/* Ride attribute badges */}
                    {(ride.musicPreference || ride.hasAc !== null || ride.hasTrunkSpace !== null || ride.vehicleType) && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {ride.vehicleType && (
                          <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-blue-100">
                            🚗 {ride.vehicleType.charAt(0) + ride.vehicleType.slice(1).toLowerCase()}
                          </span>
                        )}
                        {ride.musicPreference === "MUSIC_ALLOWED" && (
                          <span className="inline-flex items-center gap-1 bg-purple-50 text-purple-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-purple-100">
                            🎵 Music
                          </span>
                        )}
                        {ride.musicPreference === "NO_MUSIC" && (
                          <span className="inline-flex items-center gap-1 bg-zinc-100 text-zinc-600 text-xs font-semibold px-2 py-0.5 rounded-full border border-zinc-200">
                            🔇 No Music
                          </span>
                        )}
                        {ride.hasAc === true && (
                          <span className="inline-flex items-center gap-1 bg-sky-50 text-sky-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-sky-100">
                            ❄️ AC
                          </span>
                        )}
                        {ride.hasTrunkSpace === true && (
                          <span className="inline-flex items-center gap-1 bg-orange-50 text-orange-700 text-xs font-semibold px-2 py-0.5 rounded-full border border-orange-100">
                            🧳 Trunk
                          </span>
                        )}
                      </div>
                ...
                {successState ? (
                  ...
                  >
                    ...
                    <h2 id="rideDetailsTitle" className="text-3xl font-bold text-zinc-900">
                      {successState === "edit" ? "Ride Updated!" : successState === "offer" ? "Offer Sent!" : successState === "stopRequest" ? "Stop Requested!" : "Ride Confirmed!"}
                    </h2>
                    <p className="text-zinc-500 max-w-sm">
                      {successState === "edit"
                        ? "Your ride details were saved successfully."
                        : successState === "offer"
                          ? "Your price offer has been sent to the driver."
                          : successState === "stopRequest"
                            ? "Your stop request has been sent to the driver for review."
                        ...
                        <>
                          ...
                          {showOfferForm && (
                            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 space-y-3">
                              <h4 className="font-semibold text-blue-900">Send a Price Offer</h4>
                              <p className="text-sm text-blue-700">Suggest a different price per seat to the driver.</p>
                              {offerError && (
                                <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2">{offerError}</div>
                              )}
                              <div className="flex items-center gap-3">
                                <div className="relative flex-1">
                                  ...
                                  <input
                                    ...
                                    value={offerPriceDollars}
                                    onChange={(e) => setOfferPriceDollars(e.target.value)}
                                    className="w-full bg-white border border-blue-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl p-3 pl-7 text-zinc-900 outline-none"
                                  />
                                </div>
                                <button
                                  onClick={() => handleSendRideOffer(selectedRide.id)}
                                  disabled={offerSubmitting}
                                  className="px-5 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors disabled:opacity-60 whitespace-nowrap"
                                >
                          ...
                          {showStopRequestForm && (
                            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 space-y-3">
                              <h4 className="font-semibold text-amber-900">Request a Custom Stop</h4>
                              <p className="text-sm text-amber-700">Ask the driver to pick you up or drop you off at specific locations.</p>
                              {stopRequestError && (
                                <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-xl px-3 py-2">{stopRequestError}</div>
                              )}
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div>
                                  <label className="block text-sm font-medium text-amber-800 mb-1">Pickup Location</label>
                                  <input
                                    type="text"
                                    placeholder="Where to pick you up..."
                                    value={stopPickupText}
                                    onChange={(e) => setStopPickupText(e.target.value)}
                                    className="w-full bg-white border border-amber-200 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl p-3 text-zinc-900 outline-none text-sm"
                                  />
                                </div>
                                <div>
                                  <label className="block text-sm font-medium text-amber-800 mb-1">Dropoff Location</label>
                                  <input
                                    type="text"
                                    placeholder="Where to drop you off..."
                                    value={stopDropoffText}
                                    onChange={(e) => setStopDropoffText(e.target.value)}
                                    className="w-full bg-white border border-amber-200 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl p-3 text-zinc-900 outline-none text-sm"
                                  />
                                </div>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-amber-800 mb-1">Note (optional)</label>
                                <textarea
                                  placeholder="Any details for the driver..."
                                  value={stopRiderNote}
                                  onChange={(e) => setStopRiderNote(e.target.value)}
                                  rows={2}
                                  className="w-full bg-white border border-amber-200 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 rounded-xl p-3 text-zinc-900 outline-none text-sm"
                                />
                              </div>
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => handleSendStopRequest(selectedRide.id)}
                                  disabled={stopRequestSubmitting}
                                  className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-medium rounded-xl transition-colors disabled:opacity-60"
                                >
                                  {stopRequestSubmitting ? "Sending..." : "Send Request"}
                                </button>
                                <button
                                  onClick={() => { setShowStopRequestForm(false); setStopRequestError(null); }}
                          ...
                          {/* Action buttons row */}
                          {!showOfferForm && !showStopRequestForm && (
                            <div className="flex flex-wrap gap-3 justify-end items-center">
                              <button
                                onClick={() => setShowStopRequestForm(true)}
                                className="px-4 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 font-medium rounded-xl transition-colors border border-amber-200 text-sm"
                              >
                                Request Stop
                              </button>
                              <button
                                onClick={() => setShowOfferForm(true)}
                                className="px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 font-medium rounded-xl transition-colors border border-blue-200 text-sm"
                              >
                                Send Offer
                              </button>
          ...
          >
            <div className="p-6 md:p-8">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="af-music" className="block text-sm font-semibold text-emerald-800 mb-2">
                      Music Preference
                    </label>
                    <select
                      id="af-music"
                      value={pendingAdvancedFilters.musicPreference}
                      onChange={(e) =>
                  ...
                  <div>
                    <label htmlFor="af-vehicle-type" className="block text-sm font-semibold text-emerald-800 mb-2">
                      Vehicle Type
                    </label>
                    <select
                      id="af-vehicle-type"
                      value={pendingAdvancedFilters.vehicleType}
                      onChange={(e) =>
                        setPendingAdvancedFilters({
                          ...pendingAdvancedFilters,
                  ...
                  <div>
                    <label htmlFor="af-has-ac" className="block text-sm font-semibold text-emerald-800 mb-2">
                      AC Availability
                    </label>
                    <select
                      id="af-has-ac"
                      value={pendingAdvancedFilters.hasAc}
                      onChange={(e) =>
                        setPendingAdvancedFilters({
                          ...pendingAdvancedFilters,
                          hasAc: e.target.value,
                        })
                      }
                      className="w-full bg-white border border-zinc-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl p-3 text-zinc-900 shadow-sm outline-none"
                    >
                  ...
                  <div>
                    <label htmlFor="af-has-trunk-space" className="block text-sm font-semibold text-emerald-800 mb-2">
                      Trunk Space Availability
                    </label>
                    <select
                      id="af-has-trunk-space"
                      value={pendingAdvancedFilters.hasTrunkSpace}
                      onChange={(e) =>
                        setPendingAdvancedFilters({
                          ...pendingAdvancedFilters,
                          hasTrunkSpace: e.target.value,
                        })
                      }
                      className="w-full bg-white border border-zinc-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl p-3 text-zinc-900 shadow-sm outline-none"
                    >
