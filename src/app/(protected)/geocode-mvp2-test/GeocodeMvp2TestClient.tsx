...
export default function GeocodeMvp2TestClient() {
  ...
  async function submitRideCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const earliestDepartAtIso = localToIso(rideCreate.earliestDepartAtLocal);
      const latestDepartAtIso = localToIso(rideCreate.latestDepartAtLocal);

      if (!earliestDepartAtIso || !latestDepartAtIso) {
        setRideCreateResult({
          status: 400,
          body: {
      ...
      const payload = {
        ...
        earliestDepartAt: earliestDepartAtIso,
        latestDepartAt: latestDepartAtIso,
  ...
  async function submitTripCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const earliestDesiredAtIso = localToIso(tripCreate.earliestDesiredAtLocal);
      const latestDesiredAtIso = localToIso(tripCreate.latestDesiredAtLocal);

      if (!earliestDesiredAtIso || !latestDesiredAtIso) {
        setTripCreateResult({
          status: 400,
          body: {
            error: "Validation Error",
            message: "Earliest and latest desired departure must be valid date-time values.",
          },
        });
        return;
      }
      ...
      const payload = {
        ...
        earliestDesiredAt: earliestDesiredAtIso,
        latestDesiredAt: latestDesiredAtIso,
