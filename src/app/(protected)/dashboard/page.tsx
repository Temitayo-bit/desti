...
export default function DashboardPage() {
  ...
  interface SuggestedMatch {
    ...
    ride: {
      ...
      driver?: { name: string | null; profilePictureUrl: string | null } | null;
    };
  }
  const [suggestedMatches, setSuggestedMatches] = useState<SuggestedMatch[]>([]);
  const [matchActions, setMatchActions] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    async function fetchMatches() {
      try {
        ...
        if (!res.ok) return;
        const data = (await res.json()) as { items: SuggestedMatch[] };
        if (!controller.signal.aborted) {
          setSuggestedMatches(data.items ?? []);
        }
      } catch {
        // Best-effort fetch
      }
  ...
  async function runMatchAction(matchId: string, action: "accept" | "reject") {
    setMatchActions((prev) => ({ ...prev, [matchId]: action }));
    try {
      const res = await fetch(`/api/matches/${matchId}/${action}`, { method: "POST" });
      if (res.ok) {
        setSuggestedMatches((prev) => prev.filter((m) => m.id !== matchId));
        setActionNotice({ type: "success", text: `Match ${action === "accept" ? "accepted" : "rejected"}.` });
        await refreshDashboard({ silent: true });
      } else {
        const data = await res.json().catch(() => null);
        setActionNotice({ type: "error", text: data?.message ?? `Failed to ${action} match.` });
      }
    } catch {
      setActionNotice({ type: "error", text: "Network error. Please try again." });
    } finally {
      ...
      <section className="space-y-8">
        ...
        {loading ? (
          ...
          <>
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-zinc-500">
                      Rides that match your trip requests. Accept to book or reject to skip.
                    </p>
                  </div>
                  <span className="inline-flex items-center rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-purple-700">
                    {suggestedMatches.length} {suggestedMatches.length === 1 ? "match" : "matches"}
                  </span>
                </div>

                <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
                  {suggestedMatches.map((match) => {
                    const busyAction = matchActions[match.id] ?? null;
                    const isBusy = Boolean(busyAction);
                    const matchScore = Math.round(match.scoreSnapshot * 100);

                    return (
                      <article key={match.id} className={`${CARD_CLASS} transition-all hover:border-purple-400/50 hover:shadow-md`}>
                        {/* Match score badge */}
                        ...
                        <div className="mb-3 rounded-xl bg-zinc-50 p-3">
                          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-zinc-400">Your request</p>
                          <p className="text-sm font-medium text-zinc-700">
                            {match.tripRequest.originText} → {match.tripRequest.destinationText}
                          </p>
                        </div>

                        {/* Matched ride */}
                        <div className="mb-4">
                          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-emerald-600">Matched ride</p>
                          <div className="flex items-start gap-2">
                            <MapPinIcon className="mt-0.5 shrink-0 text-emerald-500" />
                            <div className="min-w-0">
                              <p className="truncate font-semibold text-zinc-900">{match.ride.originText}</p>
                              <p className="text-xs text-zinc-400">to</p>
                              <p className="truncate font-semibold text-zinc-900">{match.ride.destinationText}</p>
                            </div>
                          </div>
                        </div>

                        <div className="mb-5 flex flex-wrap gap-3 text-sm text-zinc-600">
                          <span className="inline-flex items-center gap-1.5">
                            <CalendarIcon />
                            {formatTimeRange(match.ride.earliestDepartAt, match.ride.latestDepartAt)}
                          </span>
                          <span className="inline-flex items-center gap-1.5">
                            <UsersIcon />
                            {match.ride.seatsAvailable} seats left
                          </span>
                          <span className="text-lg font-bold text-emerald-600">
                            $ {(match.ride.priceCents / 100).toFixed(2)}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <button
                            type="button"
                            disabled={isBusy}
                            onClick={() => void runMatchAction(match.id, "accept")}
                          ...
                          <button
                            type="button"
                            disabled={isBusy}
                            onClick={() => void runMatchAction(match.id, "reject")}
                            className="w-full rounded-2xl border border-zinc-300 px-4 py-3 text-sm font-semibold text-zinc-900 hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {busyAction === "reject" ? "Rejecting..." : "Skip"}
                          </button>
                        </div>
                      </article>
                    );
            ...
            {/* Active Rides You're Driving */}
            {(dashboard?.upcoming.ridesDriving ?? []).length > 0 && (
              <section className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold tracking-tight text-zinc-900 md:text-3xl">
                    ...
                    <p className="mt-1 text-sm text-zinc-500">
                      Rides you posted as driver that are still active.
                    </p>
                  </div>
                  <span className="inline-flex items-center rounded-full bg-zinc-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-zinc-600">
                    {(dashboard?.upcoming.ridesDriving ?? []).length}{" "}
                    {(dashboard?.upcoming.ridesDriving ?? []).length === 1 ? "ride" : "rides"}
                  </span>
                </div>

                <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
                  {(dashboard?.upcoming.ridesDriving ?? []).map((ride) => (
                    <article key={ride.id} className={`${CARD_CLASS} transition-all hover:border-emerald-500/50 hover:shadow-md`}>
                      <div className="mb-4 flex items-start justify-between gap-3">
                        ...
                        <span className={statusPill("green")}>ACTIVE</span>
                      </div>
                      ...
                      <div className="mt-4 flex items-end justify-between">
                        <span className="inline-flex rounded-xl bg-zinc-100 px-3 py-1 text-xs tracking-tight text-zinc-500">
                          {toDistanceLabel(ride.distanceCategory)}
                        </span>
                        <p className="text-2xl font-bold tracking-tight text-emerald-600">
